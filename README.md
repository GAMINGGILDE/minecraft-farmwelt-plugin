...
Der optionale Bereich `farmworlds.<id>.reset.notifications` bereitet pro Farmwelt Countdown-, Start-, Erfolgs-, Fehler- und Evakuierungsnachrichten vor. Warning-Zeiten nutzen ebenfalls positive `m`-/`h`-/`d`-Werte; ungültige Einträge werden einzeln ignoriert, Duplikate entfernt und die übrigen Schwellen absteigend sortiert. `{world}`, `{time}` und `{next-reset}` sind als zukünftige Platzhalter dokumentiert. In Phase 5.1 wird diese Konfiguration nur geladen: Es werden noch keine Nachrichten versendet und keine Notification-Daten in `reset-state.yml` gespeichert. Details und Defaults stehen im [Admin Guide](docs/ADMIN_GUIDE.md).
