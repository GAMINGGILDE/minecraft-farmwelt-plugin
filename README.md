...
Der optionale Bereich `farmworlds.<id>.reset.notifications` steuert pro Farmwelt die Countdown-Warnungen vor automatisch geplanten Resets. Warning-Zeiten nutzen positive `m`-/`h`-/`d`-Werte; ungültige Einträge werden einzeln ignoriert, Duplikate entfernt und die übrigen Schwellen absteigend sortiert. Jede Schwelle wird pro `nextReset`-Termin höchstens einmal versendet. Nach Neustart oder Reaktivierung wird höchstens die zeitlich nächste bereits erreichte Warnung nachgeholt, niemals eine alte Nachrichtenserie. `{world}`, `{time}` und `{next-reset}` werden im konfigurierten Warning-Text ersetzt. Der Versandstatus bleibt rein transient und verändert `reset-state.yml` nicht. Start-, Erfolgs-, Fehler- und Evakuierungsmeldungen sind weiterhin nur vorbereitet. Details und Defaults stehen im [Admin Guide](docs/ADMIN_GUIDE.md).
...
Nach dem vollständigen Pluginstart wartet Farmwelt einmalig 60 Sekunden. Anschließend werden überfällige aktivierte Reset-Pläne in stabiler Konfigurationsreihenfolge nacheinander über dieselbe vollständige Reset-Pipeline ohne `--dragon` nachgeholt. Vor jeder Welt wird der aktuelle `nextReset` erneut geprüft; ein zwischenzeitlich manuell erledigter Reset wird daher nicht doppelt ausgeführt. Fehler und `ALREADY_RUNNING` blockieren die folgenden Welten nicht. Nach der Startup-Nachholung übernimmt genau ein regulärer Scheduler und prüft aktivierte Reset-Pläne alle 60 Sekunden. Ein fehlgeschlagener Reset verschiebt `nextReset` nicht und kann beim nächsten regulären Tick erneut versucht werden. Countdown, Broadcasts und Retry-Backoff sind noch nicht enthalten; die Startup-Policy ist in Version 2 fest `IMMEDIATE` und nicht konfigurierbar.
